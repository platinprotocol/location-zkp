...
\abstract{
Many location based services authorize a user by assessing whether or not the user is within a given range of the service. To assess this range these systems request the user's geographical coordinates and often store them for later analysis. A system is proposed where the service authorization is based on a zero knowledge verification of a commitment. The commitment has no statement of the geographical coordinates, yet can be reliably verified to prove that the user is within range of the service eligibility. The service has the assurance required to deliver the service while having zero knowledge of the user's geographical coordinates.

This paper shares the ZK procedure for a circle with radius R. A non-interactive and interactive protocol is defined that works for any arbitrary location in relation to this circle. This proof is sufficient for many use cases and preserves the users privacy. 
