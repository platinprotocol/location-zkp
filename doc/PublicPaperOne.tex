...
The zero knowledge location We present two sets of equations illustrated with a diagram and followed by notes and decisions. 

The diagram describes a circle with geometric radius R. A node positioned inside the circle can calculate a mathematical commitment and share it. In the "green" cases the difference is greater than zero, in the "red" cases it is less than zero. 

The equations are presented in two sets and outline all steps required to perform the full procedure, from commitment to verification. A protocol is specified where a verifier can process that commitment and determine the difference. An interactive  and non-interactive protocol is defined, each presented as a series of equations. This proof is sufficient for many use cases, efficient, supports large scale analytics, and preserves users' privacy.

The notes and decisions section records issues related to the equations and diagram. Each note discusses a mathematical decision that we have made. Some of these decisions are not yet reflected in the equations and protocols shared in this paper. 
...
\begin{figure}[!htb]
	...
	\caption{Two users request authorization for a location based service. The center of the circle and its preset radius define the range within which users can be authorized for the service. With data known only to itself, each user calculates a mathematical commitment based on its radius, distance and difference.  For green the difference is greater than zero. For red the difference is less than zero. The mathematical commitment is verified without revealing the user's geospatial coordinates by applying the zero knowledge protocol below.}
\end{figure}
...
Each note tackles an issue related to zero knowledge proofs of location. Some of these decisions are not reflected in the equations or protocols presented above. The notes relate generally to our work on the zero knowledge proof of location and their contents may be integrated with the main body of the paper in future. 
...
\subsection{Not-at-Location Proof}
